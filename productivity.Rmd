...
```{r}
npp_dat <- list('Central Italy' = rome_npp, 'Southwest US' = sw_npp, 'Southeast US' = se_npp, 'Yautepec Valley' = yautepec_npp, 'Fertile Crescent' = crescent_npp, 'Basin of Mexico' = bom_npp, 'Santa Valley' = santa_npp) %>%
...
ggsave('npp_dist.png', height = 4, width = 6)
```
...
ggplot(land) + 
 ...
 # geom_sf(data = goode_without, fill = "white", color = "NA") +
  #geom_sf(data = goode_outline, fill = NA, color = "gray", size = 0.5/.pt) +
    #geom_sf(data = pts, size = 3/.pt, aes(color = rowname)) +
      geom_sf(data = bnds, aes(fill = rowname, color = rowname)) +
 ...
 # ) +
  ...
  theme_bw() +
  theme(legend.position = 'none')
ggsave('case_studies.png', height = 3, width = 6.5)
```
```{r}
bnds <- list('Central Italy' = st_zm(rome), 'Southwest US' = sw_boundary, 'Southeast US' = se_boundary, 'Yautepec Valley' = yautepec, 'Fertile Crescent' = crescent, 'Basin of Mexico' = bom, 'Santa Valley' = santa)  %>%
  map(st_geometry) %>%
  do.call(c, .) %>%
  st_sf(row.names = c('Central Italy', 'Southwest US', 'Southeast US', 'Yautepec Valley', 'Fertile Crescent', 'Basin of Mexico', 'Santa Valley')) %>%
  rownames_to_column() %>%
